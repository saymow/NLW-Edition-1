...
const CreatePoint = () => {
  ...
  const [selectedItems, setSeletectedItems] = useState<number[]>([]);
  ...
  useEffect(() => {
    ...
    if (!id) {
      alert("Você precisa está logado para criar um ponto de coleta.");
  ...
  useEffect(() => {
    axios
      ...
      .then((body) => {
        const ufInitials = body.map((uf) => uf.sigla);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => response.data)
      .then((body) => {
        const cityName = body.map((city) => city.nome);
  ...
  function handleSelectItem(id: number) {
    ...
    if (alreadySelected >= 0) {
      ...
      setSeletectedItems(filteredItems);
    } else {
      setSeletectedItems([...selectedItems, id]);
    }
  }
